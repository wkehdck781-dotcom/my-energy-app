...
def loss_profile_from_excel(pump_rated_shaft_kw):
    ...
    if load_loss_fraction < 0:
        raise ValueError(
            "현재 용량의 엑셀 손실 모델이 성립하지 않습니다. "
            "제조사 부분부하 효율 데이터를 사용하세요."
        )
...
def calculate_all(pump_rated_shaft_kw, flow_pct, mode, nameplate_efficiency_pct=None):
    ...
    if nameplate_efficiency_pct is None:
        ...
        if not 50.0 <= nameplate_efficiency_pct < 100.0:
            raise ValueError("명판 모터 효율은 50~100% 사이로 입력하세요.")
...
st.markdown('<div class="app-subtitle">사내 입찰·기술제안용 검증 시스템 (폰트·회수기간·예외처리 통합)</div>', unsafe_allow_html=True)
...
if in_pm > 3000:
    st.sidebar.warning(
        ...
        "표 끝값 기준의 추정치이므로, 실제 입찰 시 제조사 성적서 효율을 직접 입력해 주세요."
    )
...
st.markdown(
    ...
    <div class="summary-grid">
        <div class="summary-card">
            <div class="summary-label">밸브 제어 소비전력</div>
...
if show_losses:
    ...
    st.info("원본 엑셀 재현식: 고정손은 n, 부하손은 n²로 계산합니다. IE3 또는 명판 효율 선택 시에는 이 부분부하 손실 곡선을 유지하고 정격 총손실만 변경합니다.")
...
if show_compare:
    ...
    if use_nameplate:
        st.caption("명판 효율 직접 입력을 사용 중이므로 두 기준 모두 같은 명판 효율을 적용합니다.")

if show_ie3_basis:
    st.markdown("### IE3 적용 기준 및 근거")
    st.markdown(
        """
        - **IEC 60034-30-1**은 선로 전원으로 운전하는 AC 모터의 IE 효율등급을 다룹니다.
        - 일반 적용 범위는 **0.12~1,000 kW, 1 kV 이하**입니다.
        - 인버터, 케이블, 필터 및 고조파로 인한 시스템 손실은 이 모터 효율등급에 포함되지 않습니다.
        - 따라서 고압 또는 1 MW 초과 모터는 이 화면의 IE3 표를 비교용 참조로만 사용하고, 실제 제안·검증에는 **현장 명판 및 제조사 시험성적서 효율**을 우선 입력해야 합니다.
        - 이 프로그램의 IE3 모드는 유량표·인버터 효율·부분부하 손실곡선은 엑셀과 동일하게 유지하고, 모터 정격효율만 IE3 참조값으로 바꿉니다.

        공식 범위 확인: [IEC 60034-30-1](https://webstore.iec.ch/en/iec_catalog/product/preview/?id=L3B1Yi9wZGYvcHJldmlldy9pbmZvX2llYzYwMDM0LTMwLTF7ZWQxLjB9Yi5wZGY%3D)
        """
    )
